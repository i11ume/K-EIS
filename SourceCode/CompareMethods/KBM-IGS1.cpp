...
//        debug(mi,id);
    ...
    vector<int> multi_question_method2(int type, int K, int B) {
    ...
    void solve_multi(vector<int> T, int &score, int &score1, int &score2, int &score3,
                     double &t1, double &t2, double &t3) {
        init_cas(T);
        clock_t t;
        t = clock();
        vector<int> tmp = multi_question_method2(1, K, B);
        score1 = get_score(tmp, T);
        t1 = (clock() - t) * 1.0 / CLOCKS_PER_SEC;
        tmp = multi_question_method2(3, K, 30);
        score2 = get_score(tmp, T);
        t2 = (clock() - t) * 1.0 / CLOCKS_PER_SEC - t1;
        score3 = get_score(multi_question_method1(1, K), T);
        t3 = (clock() - t) * 1.0 / CLOCKS_PER_SEC - t2 - t1;
        vector<int> tmp_vec;
        score = get_score(tmp_vec, T);
//        if (get_score(tmp_vec, T) > score1 || score1 != score2)
...
void solve() {
    ...
    string query_log = "KBMIGS_" + TMP + "_query_log.txt";
    ...
    for (int i = 0; i < cas; i++) {
        ...
        if (tmp.size() == 1) {
            ...
            G.solve_multi(tmp, score, score1, score2, score3, t1, t2, t3);
        }
    }
